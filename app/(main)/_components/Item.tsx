...
export function Item({ id, label, onClick, icon: Icon, active, documentIcon, isSearch, level = 0, onExpand, expanded }: ItemProps) {
  ...
  const onArchive = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    toast.promise(promise, {
      loading: "Memindahkan ke tempat sampah...",
      success: "Catatan dipindahkan ke tempat sampah!"
    })
  }
  ...
  const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    event.stopPropagation()
    if (!id) return
    const promise = create({ title: "Tanpa Judul", parentDocument: id })
    .then((documentId) => {
      if (!expanded) {
        onExpand?.()
      }
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-60" align="start" side="right" forceMount>
              ...
              <div className="text-xs text-muted-foreground p-2">
                Terakhir di edit oleh: {user?.fullName}
              </div>
            </DropdownMenuContent>
          </DropdownMenu>
